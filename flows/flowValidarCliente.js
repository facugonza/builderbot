  ...
  .addAnswer(
    ...
    async (ctx, { fallBack ,state }) => {
        ...
        if (importeRegex.test(ctx.body)) {
          ...
          return fallBack(
            "⚠️ ¿Puedes Verificar el numero ingresado ? Gracias."
          );
        } 
  ...
  .addAnswer(
    ...
    async (ctx, { fallBack ,state}) => {
        ...
        if (fechaNacimientoRegex.test(ctx.body)) {
          ...
          return fallBack(
            "⚠️ ¿Puedes Verificar la fecha ingresada. *EJ: 01/09/1990*  ? Gracias."
          );
        }  
    }
  )
  .addAnswer(
    ...
    async (ctx, { fallBack ,state}) => {
        ...
        if (ultimosCuatroDigitosRegex.test(ctx.body)) {
          ...
          return fallBack(
            "⚠️ ¿Puedes Verificar los digitos ingresados. *EJ: 1234*  ? Gracias."
          );
        }  
    }
  )  
