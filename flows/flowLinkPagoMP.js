...
const generatePayLinkVisa = async (amount,description,indentifier) => {
    ...
    const datosPago = {
      ...
      items: [
        {
          amount: amount, // Convertimos el monto a string con 2 decimales
        ...
        try{
            ...
            if (cliente.hasVisaSummary){
                ...
                if (cliente.minvisaamonut){
                    const payLinkMin   = await generatePayLinkVisa(cliente.minvisaamonut,"Pago Minimo DATA VISA",cliente.minvisaamonutidentifier);        
                    console.log("payLinkMin"   + payLinkMin);
                ...
                if (cliente.totalvisaamonut){
                    const payLinkTotal = await  generatePayLinkVisa(cliente.totalvisaamonut,"Pago Total DATA VISA",cliente.totalvisaamonutidentifier);     
                    console.log("payLinkTotal"   + payLinkTotal);
                ...
                if(messagesVISALinkPays.length > 0){
